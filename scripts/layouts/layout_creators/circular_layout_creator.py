...
def create_circular_layout(dataset, network, clustering_strategy, config, is_aggregated, resolve_overlaps):
    ...
    if is_aggregated:
        file_name += f'{config.layout_type.name.lower()}-circular-clustered.geojson'
        network.write_to_disk(f'../geo-net-app/public/{dataset}/{file_name}', include_hulls=False, include_labels=True)
    else:
        file_name += f'{config.layout_type.name.lower()}-circular.geojson'
        network.write_to_disk(f'../geo-net-app/public/{dataset}/{file_name}', include_hulls=False, include_labels=True)
    return
